...
export function ChallengeEditor({ challenge }: { challenge: ChallengeRecord }) {
  ...
  const tone = status === "saved" ? "ok" : status === "unsaved" || status === "saving" ? "warn" : "danger";
  return <div><section className="page-head"><div className="page-head-copy"><p className="page-context">Challenge capture</p><h1 className="page-title">{draft.name || "Untitled challenge"}</h1><p className="page-lede">Record the solve once here. The write-up is drafted from these notes.</p></div><div className="page-head-actions"><span role="status" className="save-state" data-tone={tone}>{statusText}</span><Button variant="outline" onClick={() => void flush()} disabled={busy || status === "saving" || status === "saved" || status === "conflict"}>Save now</Button><Button onClick={() => void openWriteup()} disabled={busy || openingWriteup || status === "conflict"}>{openingWriteup ? "Opening…" : "Generate / review write-up"}</Button></div></section>
    {message && <div role="alert" className="alert alert-danger mt-6">{message}{status === "error" && <button type="button" onClick={() => { if (savedEvidence.current !== JSON.stringify(evidence.map(({ id, caption }) => ({ id, caption })))) void updateImages(evidence); else if (saved.current !== JSON.stringify(latest.current)) void flush(); else window.location.reload(); }} className="ml-2">Retry or reload</button>}{status === "conflict" && <button type="button" onClick={() => window.location.reload()} className="ml-2">Reload latest</button>}</div>}
    <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_340px]"><section className="panel space-y-6 p-6 sm:p-7"><h2 className="panel-heading">Challenge details</h2>
      ...
      <div><Field label="How we solved it" id="source-notes"><textarea id="source-notes" className="input mono min-h-64" value={draft.source_notes} onChange={event => change("source_notes", event.target.value)} maxLength={100000} placeholder="The full solve: clues, steps, commands, outputs, and how you found the flag" /></Field><p className="field-hint">One detailed account is enough. Gemini organizes these notes into the report sections.</p><details className="subpanel mt-3 p-3 t-small"><summary className="font-medium">Used an LLM to solve it? Get the procedure from that chat</summary><p className="mt-3 t-muted">Paste this prompt into the same chat, then paste its answer above. Check the answer against what you actually did.</p><p className="subpanel-tint mt-3 select-text p-3">{solveHistoryPrompt}</p><Button className="mt-3" size="sm" variant="outline" onClick={() => void copySolvePrompt()}>{promptCopied ? "Prompt copied" : "Copy prompt"}</Button></details></div>
      <Field label="Commands, code, and output" id="commands"><textarea id="commands" className="input mono min-h-44" value={draft.commands} onChange={event => change("commands", event.target.value)} maxLength={50000} placeholder="Keep the exact commands and output here" /></Field>
      <Field label="Flag (optional)" id="flag"><input id="flag" className="input flag-input" value={draft.flag} onChange={event => change("flag", event.target.value)} maxLength={2000} autoComplete="off" placeholder="CTF{...}" /></Field>
    </section><aside className="panel h-fit p-6" onDrop={onDrop} onDragOver={event => event.preventDefault()} onPaste={onPaste}><h2 className="panel-heading">Screenshots <span className="count">{evidence.length}</span></h2><p className="mt-1 t-small t-muted">Paste, drop, or select images. PNG, JPEG, and WebP up to 5 MB each.</p><input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" multiple className="sr-only" aria-label="Choose screenshots" onChange={(event: ChangeEvent<HTMLInputElement>) => { if (event.target.files) void uploadFiles(event.target.files); }} /><Button className="mt-4 w-full" variant="outline" onClick={() => inputRef.current?.click()} disabled={busy || status === "conflict"}>{busy ? "Working…" : "Add screenshots"}</Button><div className="mt-5 space-y-4">{evidence.map((item, index) => <div key={item.id} className="subpanel p-3">{urls[item.id] ? <Image src={urls[item.id]} alt={item.caption || `Screenshot ${index + 1}`} width={640} height={360} unoptimized className="max-h-52 w-full rounded-md border border-[var(--line)] object-contain" /> : <div className="subpanel-tint p-8 text-center t-small t-muted">Loading image…</div>}<label className="field-label mt-3" htmlFor={`caption-${item.id}`}>Caption</label><input id={`caption-${item.id}`} className="input t-small" value={item.caption} onChange={event => { setEvidence(current => current.map(image => image.id === item.id ? { ...image, caption: event.target.value } : image)); setStatus("unsaved"); }} disabled={busy || status === "conflict"} onBlur={() => { if (savedEvidence.current !== JSON.stringify(evidence.map(({ id, caption }) => ({ id, caption })))) void updateImages(evidence); }} maxLength={500} placeholder="What does this show?" /><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="small-action" disabled={busy || index === 0 || status === "conflict"} onClick={() => { const next = [...evidence]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; void updateImages(next); }}>↑ Up</button><button type="button" className="small-action" disabled={busy || index === evidence.length - 1 || status === "conflict"} onClick={() => { const next = [...evidence]; [next[index + 1], next[index]] = [next[index], next[index + 1]]; void updateImages(next); }}>↓ Down</button><button type="button" className="small-action t-danger" disabled={busy || status === "conflict"} onClick={() => void deleteImage(item)}>Remove</button></div></div>)}{!evidence.length && <div className="subpanel-dashed p-8 text-center t-small">Drop images here as you solve the challenge.</div>}</div></aside></div>
  </div>;
}
function Field({ label, id, children }: { label: string; id: string; children: React.ReactNode }) { return <div><label htmlFor={id} className="field-label">{label}</label>{children}</div>; }
