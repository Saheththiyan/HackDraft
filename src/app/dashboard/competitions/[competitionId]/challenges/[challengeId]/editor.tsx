...
export function ChallengeEditor({ challenge }: { challenge: ChallengeRecord }) {
  ...
  return <div><section className="page-hero mt-6 p-8 sm:p-10"><p className="hero-label">Challenge capture</p><div className="mt-5 flex flex-wrap items-center justify-between gap-5"><h1 className="hero-title">{draft.name || "Untitled challenge"}</h1><div className="flex flex-wrap items-center gap-3"><span role="status" className={`text-sm ${status === "saved" ? "text-emerald-300" : status === "unsaved" || status === "saving" ? "text-amber-300" : "text-rose-300"}`}>{statusText}</span><Button variant="outline" onClick={() => void flush()} disabled={busy || status === "saving" || status === "saved" || status === "conflict"}>Save now</Button><Button onClick={() => void openWriteup()} disabled={busy || openingWriteup || status === "conflict"}>{openingWriteup ? "Opening…" : "Generate / review write-up →"}</Button></div></div>
    <p className="hero-subtitle mt-4">Capture the solve once here, then generate and review the report write-up.</p></section>
