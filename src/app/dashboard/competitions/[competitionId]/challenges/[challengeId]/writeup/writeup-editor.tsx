...
export function WriteupEditor({ challenge, initialSections, images }: {
  ...
  return <div><div className="mt-3 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-4xl font-bold">{challenge.name}</h1><p className="mt-2 text-slate-400">Generate a first draft from your saved solve notes, then edit and approve it.</p></div><div className="flex items-center gap-3"><span role="status" className={`text-sm ${state === "reviewed" || state === "saved" ? "text-emerald-300" : state === "conflict" || state === "error" ? "text-rose-300" : "text-amber-300"}`}>{({ saved: "Draft saved", unsaved: "Unsaved draft", saving: "Saving…", reviewed: "Reviewed", error: "Save failed", conflict: "Edit conflict" } as const)[state]}</span><Button variant="outline" disabled={state === "saving" || state === "conflict"} onClick={() => void flush()}>Save draft</Button><Button disabled={state === "saving" || state === "conflict" || !readyForReview(sections)} onClick={() => void approve()}>{state === "reviewed" ? "Reviewed ✓" : "Approve write-up"}</Button></div></div>
