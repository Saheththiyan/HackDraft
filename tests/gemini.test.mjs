...
test("Gemini receives source data and a five-section JSON schema, then returns its draft", async () => {
  let called = false;
  const result = await requestGeminiDraft({ key: "test-key", model: "gemini-2.5-flash", fields }, async (url, options) => {
    called = true;
...
test("Gemini rate limits return a usable message", async () => {
  const result = await requestGeminiDraft({ key: "test-key", model: "gemini-2.5-flash", fields }, async () => new Response("", { status: 429 }));
...
test("blocked or malformed Gemini responses are rejected", async () => {
  const blocked = await requestGeminiDraft({ key: "test-key", model: "gemini-2.5-flash", fields }, async () => Response.json({ candidates: [] }));
  assert.equal(blocked.ok, false);
  const malformed = await requestGeminiDraft({ key: "test-key", model: "gemini-2.5-flash", fields }, async () => Response.json({ candidates: [{ content: { parts: [{ text: "not JSON" }] } }] }));
  assert.equal(malformed.ok, false);
});

test("network failures do not leak request details", async () => {
  const result = await requestGeminiDraft({ key: "test-key", model: "gemini-2.5-flash", fields }, async () => { throw new Error("private key: test-key"); });
