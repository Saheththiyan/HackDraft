...
test("saved solve notes become an editable Gemini draft", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog", { name: "Use this AI draft?" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Keep current text" })).toBeFocused();
  ...
  await expect(page.getByRole("dialog")).not.toBeVisible();
  await expect(page.getByText("Generated draft ready to review")).toBeVisible();
  await expect(page.getByLabel("Challenge overview content")).toContainText("Find the hidden flag");
  await page.getByRole("button", { name: "Use this draft" }).click();
